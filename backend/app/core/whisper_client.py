...
def transcribe_audio_sync(
    ...
    """
    Synchronous version of transcribe_audio for use in background tasks.
    ...
    logger.info("Sending %s (%d bytes) to Groq Whisper API (sync)", filename, len(audio_bytes))
    ...
    logger.info("Received %d segments from Groq Whisper (sync)", len(segments))
    return segments
